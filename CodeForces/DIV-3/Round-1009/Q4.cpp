...
void solve() {
    ...
    unordered_set<pair<int, int>, function<size_t(const pair<int, int>&)>> st(100000, 
        [](const pair<int, int>& p) { return hash<long long>()(((long long)p.first << 32) | p.second); });

    for (int i = 0; i < n; i++) {
        int cx = x[i], cr = r[i];
        for (int dx = -cr; dx <= cr; dx++) {
            int x_coord = cx + dx;
            int max_y = sqrt(cr * cr - dx * dx);  // Only valid y-values inside the circle
            for (int dy = -max_y; dy <= max_y; dy++) {
                st.insert({x_coord, dy});
            }
        }
    }
