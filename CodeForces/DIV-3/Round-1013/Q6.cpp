...
void solve() {
    ...
    for (ll i = 0; i < n; i++) cin >> mountain[i];
    ll hlim = (d * d - 1 >= 0) ? floor(sqrt((long double)d * d - 1.0L)) : 0;
    vector<ll> dp_next(m + 1, 0), dp_curr(m + 1, 0);
    for (ll i = n - 1; i >= 0; i--) {
        vector<ll> holds;
        for (ll j = 0; j < m; j++) if (mountain[i][j] == 'X') holds.push_back(j + 1);
        if (holds.empty()) {
            dp_curr.assign(m + 1, 0);
        } else {
            vector<ll> weight(holds.size(), (i == n - 1) ? 1 : 0);
            if (i != n - 1) {
                vector<ll> pref(m + 2, 0);
                for (ll j = 1; j <= m; j++) pref[j] = (pref[j - 1] + dp_next[j]) % MOD;
                for (ll k = 0; k < holds.size(); k++) {
                    ll f = holds[k];
                    weight[k] = (pref[min(m, f + hlim)] - pref[max(0LL, f - hlim - 1)] + MOD) % MOD;
                }
            }
            dp_curr.assign(m + 1, 0);
            for (ll k = 0; k < holds.size(); k++) dp_curr[holds[k]] = (dp_curr[holds[k]] + weight[k]) % MOD;
            vector<ll> diff(holds.size() + 1, 0);
            for (ll k = 0; k < holds.size(); k++) {
                ll f = holds[k];
                ll L = lower_bound(holds.begin(), holds.end(), f - d) - holds.begin();
                ll R = upper_bound(holds.begin(), holds.end(), f + d) - holds.begin() - 1;
                if (L <= R) {
                    diff[L] = (diff[L] + weight[k]) % MOD;
                    if (R + 1 < diff.size()) diff[R + 1] = (diff[R + 1] - weight[k] + MOD) % MOD;
                    diff[k] = (diff[k] - weight[k] + MOD) % MOD;
                    diff[k + 1] = (diff[k + 1] + weight[k]) % MOD;
                }
            }
            ll cur = 0;
            for (ll k = 0; k < holds.size(); k++) {
                cur = (cur + diff[k]) % MOD;
                dp_curr[holds[k]] = (dp_curr[holds[k]] + cur) % MOD;
            }
        }
        dp_next = dp_curr;
    }
    ll ans = 0;
    for (ll j = 0; j < m; j++) if (mountain[0][j] == 'X') ans = (ans + dp_curr[j + 1]) % MOD;
    cout << ans << "\n";
}
