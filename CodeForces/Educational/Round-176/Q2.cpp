...
void solve() {
    ...
    for (ll i = 1; i <= n; ++i) {
        ll mxs = min(i, k);
        ldp[i].resize(mxs + 1, NEG);
        
        for (ll j = 0; j < ldp[i - 1].size(); ++j) {
            ldp[i][j] = max(ldp[i][j], ldp[i - 1][j]);
        }
        
        for (ll j = 1; j <= mxs; ++j) {
            if (j - 1 < ldp[i - 1].size() && ldp[i - 1][j - 1] != NEG) {
    ...
    for (ll i = n - 1; i >= 0; --i) {
        ll mxs = min(n - i, k);
        rdp[i].resize(mxs + 1, NEG);
        
        for (ll j = 0; j < rdp[i + 1].size(); ++j) {
            rdp[i][j] = max(rdp[i][j], rdp[i + 1][j]);
        }
        
        for (ll j = 1; j <= mxs; ++j) {
            if (j - 1 < rdp[i + 1].size() && rdp[i + 1][j - 1] != NEG) {
    ...
    for (ll i = 0; i < n; ++i) {
        ...
        if (i == 0) {
            ...
            for (ll x = 1; x <= min(k - 1, ls); ++x) {
                ll y = k - x;
                if (y < 1 || y > rs) continue;
                
                if (x < ldp[i].size() && y < rdp[i + 1].size()) {
                    if (ldp[i][x] != NEG && rdp[i + 1][y] != NEG) {
                        cur = max(cur, a[i] + ldp[i][x] + rdp[i + 1][y]);
                    }
                }
            }
        }
