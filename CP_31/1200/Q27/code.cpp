...
void solve() {
    ...
    string t; cin>>t;
    ll c0 = 0, c1 = 0;
    ll cs0 = count(s.begin(), s.end(), '0');
    ll cs1 = count(s.begin(), s.end(), '1');
    ll ct0 = count(t.begin(), t.end(), '0');
    ll ct1 = count(t.begin(), t.end(), '1');
    if(cs0 != ct0 || cs1 != ct1){
        cout << "NO" << nl;
        return;
    }
    
    ll allsame = 1, alldiff = 1;
    for(ll i = 0; i < n; i++){
        if(s[i] == '0') c0++;
        if(s[i] == '1') c1++;
        if(s[i] == t[i]) alldiff = 0;
        if(s[i] != t[i]) allsame = 0;
        if(c0 == c1){
            if(allsame == 0 && alldiff == 0){ // neither all same nor all different
                cout << "NO" << nl;
                return;
            }
