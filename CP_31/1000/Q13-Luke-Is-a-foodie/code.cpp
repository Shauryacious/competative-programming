...
void solve() {
    ...
    for(ll i=1; i<n; i++){
        ...
        if(r1 < l || r < l1){ // no intersection
        // the current segment is not intersecting with the previous segment
        // it is in the left of the previous segment
        // or it is in the right of the previous segment
            cnt++;
            l = l1;
            r = r1;
        }
        else{ // intersection exists => update the intersection
            l = max(l, l1);
            r = min(r, r1);
        }
    }
