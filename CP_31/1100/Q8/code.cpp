...
void solve() {
    ...
    ll ans = llabs(x[b]-x[a]) + llabs(y[b]-y[a]);
    if(k <= 1){
        cout<<ans<<nl;
        return;
    }
    // Now we have atleast 2 major cities
    // traverse from a -> s -> t -> b
    ll as = LLONG_MAX; 
    ll tb = LLONG_MAX;

    for(ll i=1; i<=k; i++){
        as = min(as, llabs(x[a]-x[i]) + llabs(y[a]-y[i]));
        tb = min(tb, llabs(x[b]-x[i]) + llabs(y[b]-y[i]));
    }

    // Now we have the minimum distance from a to s and b to t
    ans = min(ans, (as + tb));
