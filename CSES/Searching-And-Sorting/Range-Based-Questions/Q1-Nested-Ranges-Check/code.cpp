...
void solve() {
    ...
    auto cmp = [&](auto &a, auto &b){
        if(a[0] == b[0]){ // if l is same
            return a[1] > b[1]; // sort r in decreasing order
        }
        return a[0] < b[0]; // sort l in increasing order
    };
    ...
    for(ll i=n-2; i>=0; i--){
        ...
        ll curr_idx = v[i][2];
        if(curr_r >= min_r){
            containes[curr_idx] = 1;
        }
        min_r = min(min_r, curr_r);
    }
    
    for(auto x: containes){
        cout << x << " ";
    }
    cout << nline;
...
int n; cin>>n;
    ...
    while(does_change){
        ...
        if(!temp.empty()) temp.insert(temp.begin(), v2[0]);
        v2 = temp;
        count++;
    }
