...
void solve() {
    ...
    // Check if there exist a palindrome already
    for(ll i=0; i<n; i++){
        string t = v[i];
    ...
    //1) Check for plindrome of length 1
    for(ll i=0; i<n; i++){
        if(v[i].length() == 1){
            cout << "YES" << nl;
            return;
    ...
    // Now it is garunteed that there is no palindrome
    // also no string of length 1

    //2) Check for palindrome of length 4 or 6
    ...
    //3) Check for palindrome of length 5
    for(ll i=0; i<n; i++){
        if(v[i].length() == 3){
            st.erase(st.find(v[i]));
            string t = v[i];
