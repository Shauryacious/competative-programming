...
bool canDistribute(vector<ll>& a, ll n, ll k, ll minShare) {
    ll currentSum = 0;
    ll countPeople = 1;
    
    for(int i = 0; i < n; i++) {
        if (currentSum + a[i] > minShare) {
            countPeople++;
            currentSum = a[i]; // Start new person's share
            if (countPeople > k) return false;
        } else {
            currentSum += a[i]; // Add the piece to the current person
        }
    }
...
void solve() {
    ...
    while (low <= high) {
        ...
        if (canDistribute(a, n, k, mid)) {
            result = mid; // If we can distribute with this minShare, it's a valid candidate
            low = mid + 1; // Try for a larger minShare
        } else {
            high = mid - 1; // Try for a smaller minShare
        }
    }
