...
ll next(ll curr){
    ll x = curr;
    if(a1 <= x && x <= a2){ // if x is in the range of a1 and a2
        x = a1 + a2 - x;    // then x will be a1 + a2 - x
    }
    if(b1 <= x && x <= b2){
        x = b1 + b2 - x;
    }
...
void solve() {
    ...
    vll ans(N + 1, -1);
    for(ll i = 1; i <= N; i++){
        // p = how many turns taken so far
        // curr = position of the i cow after p turns
        ll p = 1, curr = next(i);
        while(curr != i){
            curr = next(curr);
            p++;
        }
        // after p turns, the i cow will be at the same position
        // k = how many turns left
        ll k = K % p;
        // simulate the process for 'k' turns now
        curr = i;
        for(ll j = 0; j < k; j++){
            curr = next(curr);
        }
