...
void solve() {
    ...
    //if we have only one sink
    // then all the other nodes can reach the sink

    if(cnt==1){
        cout<<ans+1<<nl;
        return;
    }   
    //if we have more than one sink
    // then there is no way that all the other 
    // nodes can reach the sink
