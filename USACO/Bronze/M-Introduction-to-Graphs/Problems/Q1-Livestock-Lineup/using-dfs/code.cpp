...
void dfs(ll u, vll& visited, vector<vector<ll>>& adj, vector<string>& cows){
    visited[u] = 1;
    cout << cows[u] << nl;
    for(auto v : adj[u]){
        if(!visited[v]){
            dfs(v, visited, adj, cows);
        }
    }
}
...
void solve() {
    ...
    for(ll i = 0; i < n; i++){
        string c1, t, c2; // t = trash
        cin >> c1 >> t >> t >> t >> t >> c2;
        ll u = find(cows.begin(), cows.end(), c1) - cows.begin();
        ll v = find(cows.begin(), cows.end(), c2) - cows.begin();
        adj[u].pb(v);
        adj[v].pb(u);
    }
