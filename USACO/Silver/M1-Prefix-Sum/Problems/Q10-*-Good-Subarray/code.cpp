...
void solve() {
    ...
    map<ll, ll> m;
    // Good subarray => pref[r] - pref[l] = r - l
    // Rearranging => pref[r] - r = pref[l] - l
    // So, we need to find the number of pairs of (pref[i] - i) that are equal
    // We can do this by storing the frequency of each pref[i] - i in a map
    // Then, for each value of pref[i] - i, we can find the number of pairs of that value by using the formula nC2 => n*(n-1)/2
    for(ll i=0;i<=n;i++){
        m[pref[i] - i]++;
    }
