...
void solve() {
    ...
    ll n; cin >> n;
    vll preH(n+1, 0), preP(n+1, 0), preS(n+1, 0);
    for(ll i=0; i<n; i++){
        char c; cin >> c;
        preH[i+1] = preH[i];
        preP[i+1] = preP[i];
    ...
    for(ll i=1; i<n; i++){
        ...
        ll p = preP[i];
        ll s = preS[i];
        ll remh = preH[n] - preH[i];
        ll remp = preP[n] - preP[i];
        ll rems = preS[n] - preS[i];

        ll mxleft = max({h, p, s});
        ll mxright = max({remh, remp, rems});
        ans = max(ans, mxleft + mxright);
    }
