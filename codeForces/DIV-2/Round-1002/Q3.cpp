...
void solve() {
    ...
    // Handle base cases for n = 1 and n = 2 separately
    if(n == 1){
        cout << 1 << nl;
        return;
    }
    ...
    vll c(n, 1); // Track valid rows
    ll curr = 0;
    ll summ = accumulate(all(c), 0LL); // Count of valid rows
    ...
    for(ll j = n - 1; j >= 0; j--){
        ...
        for(ll i = 0; i < n; i++){
            if(c[i] == 1 && b[i][j] != curr){
                c[i] = 0; // Mark row as invalid
            }
        }

        summ = accumulate(all(c), 0LL); // Recalculate valid rows

        if(summ == 0){ // Stop if no valid rows left
            break;
        }
    }
