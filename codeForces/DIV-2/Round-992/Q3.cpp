...
void solve() {
    ...
    // Outer vector to hold levels
    vector<vector<vector<string>>> v(30);

    // Initialize with the first level containing the single string "1"
    v[0].push_back({"1"});

    // Generate levels
    for (int i = 1; i < 13; i++) {
        for (const auto& prevVec : v[i - 1]) {
            // Generate a new vector by incrementing each digit of strings in prevVec
            vector<string> newVec;
            for (const string& x : prevVec) {
                string s = x;
                int sz = s.size();
                string t(sz, '1'); // String of '1's with the same length as `s`
                int num = stoi(t);
                int num1 = stoi(s);
                int add = num + num1;
                s = to_string(add);
            ...
            // Prepend "1" to each string in newVec
            for (auto& s : newVec) {
                s = "1" + s;
            }

            // Add this new vector to the current level
            v[i].push_back(newVec);

            // Create a reversed vector
            vector<string> reversedVec = newVec;
            for (string& s : reversedVec) {
                reverse(s.begin(), s.end());
            }
            v[i].push_back(reversedVec);
        }
    }

    // Print all generated vectors
    for (const auto& level : v) {
        for (const auto& vec : level) {
            for (const auto& str : vec) {
                cout << str << " ";
            }
            cout << "| "; // Separate vectors within the same level
        }
        cout << endl;
    }
}
