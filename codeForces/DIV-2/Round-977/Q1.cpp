...
void solve() {
    ...
    // Initialize mid with the first two elements
    ll mid = (a[0] + a[1]) / 2;
    ...
    for(int i = 2; i < n; i++) {
        mid = max(mid, (mid + a[i]) / 2);  // Updated mid with max of current mid and the new average
    }

    cout<< 0b0010 << nline;
    
    cout << mid << nline;  // Output mid instead of min
}
