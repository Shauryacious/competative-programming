...
ll solve_dp(int pos, int c1, int c2) {
    if(pos == L) return (c1 == 0 && c2 == 0) ? 0 : -1;
    if(vis[pos][c1][c2]) return dp[pos][c1][c2];
    vis[pos][c1][c2] = true;
    ll ans = -1;
    int bx = (x >> pos) & 1, by = (y >> pos) & 1;
    for(int k_bit = 0; k_bit < 2; k_bit++){
        int sum_x = bx + k_bit + c1;
        int sum_y = by + k_bit + c2;
        int ax = sum_x & 1, ay = sum_y & 1;
        if(ax == 1 && ay == 1) continue;
        int nc1 = sum_x >> 1, nc2 = sum_y >> 1;
        ll rec = solve_dp(pos+1, nc1, nc2);
        if(rec != -1){
            ans = ((ll)k_bit << pos) | rec;
            break;
        }
    }
    dp[pos][c1][c2] = ans;
