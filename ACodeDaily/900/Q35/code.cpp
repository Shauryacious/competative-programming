...
void solve() {
    ...
    // First pass: Find the maximum difference in the range
    ll mxdiff = 0;
    for(ll i = l; i <= r; i++) {
        ll mx = 0, mn = 9;
    ...
    debug(l);
    debug(r);
    // Second pass: Find the number with the maximum difference found earlier
    ll diff = mxdiff;

    for(ll i = l; i <= r; i++) {
        string s = to_string(i);
        ll currmx = 0, currmn = 9;
        for(auto x : s) {
            ll val = x - '0';
            currmx = max(currmx, val);
            currmn = min(currmn, val);
        }
        if(currmx - currmn == diff) {
            cout << i << nl;
            return;
        }
    }
