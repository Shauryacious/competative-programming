...
void helper2(string s, ll k, map<char, ll> m, ll summ){ {
    if (s.length() <= 1) {
        return;
    }
    ...
    for (auto x : m) {
        if (x.ss % 2 == 0) {
            ls += string(x.ss / 2, x.ff);
        } else {
            ls += string(x.ss / 2 + 1, x.ff);
        }
        rs += string(x.ss / 2, x.ff);
    }
    ...
    helper2(ls, k, m1, summ);
    helper2(rs, k, m2, summ);
}
