...
// Formula to calculate lenght of all good segemnts that starts from ith index
// if len = j-i+1
// then total good segments = len*(len+1)/2
// So, we can calculate the length of all good segments that starts from ith index
...
void solve() {
    ...
    for(ll i=0; i<n; i++){
        ...
        // now the segment from i to j-1 is a good segment
        ll len = (j-1) - i + 1;
        ans += len*(len+1)/2;  // length of all good segments that starts from ith index

        sum -= a[i]; // remove the ith element from the sum
        // move ahead
    }
