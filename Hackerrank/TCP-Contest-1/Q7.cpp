...
void solve() {
    ll n, m; cin >> n >> m;
    vector<vector<ll>> adj(n); // Adjacency list for directed graph
    vector<ll> indegree(n, 0); // Indegree of each node
    
    for (ll i = 0; i < m; i++) {
        ll u, v; cin >> u >> v; u--; v--; // Convert to 0-based index
        adj[u].push_back(v);
        indegree[v]++; // Count incoming edges
    }
    debug(adj); debug(indegree);
    
    // Step 1: Compute topological order
    queue<ll> q; vector<ll> topo;
    for (ll i = 0; i < n; i++) {
    ...
    // Step 2: Compute win/lose states using DP
    vector<bool> win(n, false); // Assume all alleys are losing states (false means Rory wins)
    reverse(topo.begin(), topo.end()); // Process in reverse topological order
    for (ll cur : topo) {
        for (ll next : adj[cur]) {
            if (!win[next]) {
                win[cur] = true; // If any next state is losing, this is winning
            }
        }
    }
    debug(win);
