...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        if (s[i] == 'f') { // Check if 'f' is found
            int j = i + 1;
            if(s[j] != 'o') continue; // If 'o' is not found after 'f', skip
            while (j < n && s[j] == 'o') j++; // Count contiguous 'o's
            if (j < n && s[j] == 'd') { // Check if 'd' appears after 'o's
                count++; // Valid pattern found
            }
        }
    }
