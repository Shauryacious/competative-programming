...
const ll MAXN = 20;     // ✅ Issue: this needs to be 'const' for array size
const ll MAXSUM = 200;  // ✅ Issue: this needs to be 'const' for array size
ll dp[MAXN][2][MAXSUM]; // ✅ fixed - now sizes are compile-time constants
...
void solve() {
    ...
    auto f = [&](auto&& f, string s, ll N,  ll idx, ll tight, ll sum) -> ll {
        if (idx == N) return sum;
    ...
    ll ansl = f(f, l, n, 0, 1, 0);
    memset(dp, -1, sizeof(dp));
    ll ansr = f(f, r, m, 0, 1, 0);

    cout << ansr - ansl + check(l) << nl;
}
