...
void solve() {
    ...
    vector<vll> dp(n+2, vll(h+1, -1));
    
    // Base Case -> we have already slept of n days
    // hence the n+1th day will have 0 sleep
    for(ll j=0; j<h; j++){ 
        dp[n+1][j] = 0;
    }
    ...
    for(ll i=n; i>=1; i--){
        for(ll t=0; t<h; t++){
            ll x = a[i-1];
            // Case 1: we choose at xth hours
            ll t1 = (t + x)%h;
            ll cnt1 = (l<=t1 && t1<=r) ? 1 : 0;
            dp[i][t] = max(dp[i][t], dp[i+1][t1] + cnt1);

            // Case 2: we choose at (x-1)th hours
            ll t2 = (t + x - 1)%h;
            ll cnt2 = (l<=t2 && t2<=r) ? 1 : 0;
            dp[i][t] = max(dp[i][t], dp[i+1][t2] + cnt2);
        }
    }

    // completed 0 sleep on 1st day
    ll ans = dp[1][0];
