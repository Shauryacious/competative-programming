...
void solve() {
    ...
    //State:
    // dp[i][j] = max height till we have taken then (i-1)th player
    // and jth row player was taken last time
    ...
    for(ll i=n-1; i>=0; i--){
        dp[i][0] = max({dp[i][0], (dp[i+1][1] + a[i]), dp[i+1][0]}); // taking ith player or not taking
        dp[i][1] = max({dp[i][1], (dp[i+1][0] + b[i]), dp[i+1][1]}); // taking ith player or not taking
    }
