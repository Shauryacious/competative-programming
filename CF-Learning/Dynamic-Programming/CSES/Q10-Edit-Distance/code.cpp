...
void solve() {
    ...
    // State:
    // dp[i][j] = minimum number of operations to convert p[0...i-1] to q[0...j-1]
    vector<vector<ll>> dp(n+1, vector<ll>(m+1, 0));

    // Base Case:
    dp[0][0] = 0;
    // if both the strings are empty,
    // then the number of operations required is 0

    for(ll i=0; i<=n; i++){
        dp[i][0] = i;
        // if the second string is empty, 
        // we need to delete all the chars from the first string
    }

    for(ll i=0; i<=m; i++){
        dp[0][i] = i;
        // if the first string is empty, 
        // we need to insert all the chars from the second string
    }
    ...
    for(ll i=1; i<=n; i++){
        for(ll j=1; j<=m; j++){
            char ch1 = p[i-1]; // curr last char of the first string
            char ch2 = q[j-1]; // curr last char of the second string
            ...
            if(ch1 == ch2){
                ...
                dp[i][j] = min(
                    dp[i-1][j-1] + 1, // replace in the first string or the second string
                    min(
                        dp[i-1][j] + 1, // delete from the first string or insert in the second string
                        dp[i][j-1] + 1 // delete from the second string or insert in the first string
                    )
                );
            }
        }
