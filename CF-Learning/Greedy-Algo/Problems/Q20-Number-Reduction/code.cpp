...
void solve() {
    ...
    for(int i=0;i<n;i++){
        int curr_idx=n,new_idx=n;
        for(int j=0;j<10;j++){
            if(num[j].size())curr_idx=min(curr_idx,num[j].front());
        }
        for(int j=(i==0);j<10;j++){
            if(num[j].size() and num[j].front()-curr_idx<=k){
                t+='0'+j;
                new_idx=num[j].front();
                break;
            }
        }
        k=k-(new_idx-curr_idx);
        for(int j=0;j<10;j++){
            while(num[j].size() and num[j].front()<=new_idx){
                num[j].pop_front();
            }
        }
    }
