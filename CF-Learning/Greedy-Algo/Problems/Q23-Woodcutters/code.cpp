...
// https://www.youtube.com/watch?v=bqJvaMtMO1k
...
void solve() {
    ...
    if (n == 1) { // Special case: if there's only one tree
        cout << 1 << nl;
        return;
    }
    ll ans = 2; // The first and last trees can always be felled
    for (ll i = 1; i < n - 1; i++) {
        ll previdx = a[i - 1].ff;
        ll curridx = a[i].ff;
        ll nextidx = a[i + 1].ff;
        ll h = a[i].ss;
        if (curridx - h > previdx) {
            // The current tree can be felled to the left without affecting the previous tree
            ans++;
        } else if (curridx + h < nextidx) {
            // The current tree can be felled to the right without affecting the next tree
            ans++;
            a[i].ff += h; // Update the current position to reflect the tree has fallen to the right
        }
        // If neither condition is met, the tree remains standing
    }
    cout << ans << nl;
}
