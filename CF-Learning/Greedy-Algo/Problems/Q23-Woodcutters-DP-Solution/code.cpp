...
void solve() {
    ll n; cin >> n;
    vpll a(n + 2); // {idx, height}
    for(ll i = 1; i <= n; i++) {
        cin >> a[i].first >> a[i].second;
    }
    a[0].first = LLONG_MIN; // Sentinel for the left boundary
    a[n + 1].first = LLONG_MAX; // Sentinel for the right boundary
    ...
    for(ll i = 1; i <= n; i++) {
        ...
        ll previdx = a[i-1].first;
        ll curridx = a[i].first;
        ll nextidx = a[i+1].first;
        ll prevheight = a[i-1].second;
        ll currheight = a[i].second;
        ll nextheight = a[i+1].second;

        // Tree is felled to the left
        if(previdx < curridx - currheight) {
            dp[i][1] = max(dp[i][1], dp[i-1][0] + 1); // prev tree is standing
            dp[i][1] = max(dp[i][1], dp[i-1][1] + 1); // prev tree is felled to the left
        }
        if(previdx + prevheight < curridx - currheight) {
            dp[i][1] = max(dp[i][1], dp[i-1][2] + 1); // prev tree is felled to the right but there is space for curr tree
        }

        // Tree is felled to the right
        if(curridx + currheight < nextidx) {
