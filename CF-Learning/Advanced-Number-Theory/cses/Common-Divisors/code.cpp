...
void Construct_spf() {
    // Initialize each number with itself (each number is its own prime factor initially)
    for (ll i = 1; i <= N; i++) {
        spf[i] = i;
    }

    // Iterate over numbers starting from 2 (smallest prime)
    for (ll i = 2; i * i <= N; i++) { // Iterate i=2 --> sqrt(N)
        if (spf[i] == i) { // Current number is a prime
            for (ll j = i * i; j <= N; j += i) { // Iterate j=i^2 --> N with step i (i.e. j+=i)
                spf[j] = min(spf[j], i);
            }
        }
    }
