...
bool f(ll operations, vll a, ll sum, ll k){
    ll n = a.size();
    // CASE 1: if we only use decrement
    if(sum - operations <= k){
        return true;
    }

    // // CASE 2: if we only use set
    // ll mn = a[0];
    // if(operations >= (n-1)){
    //     if((n*mn) <= k){
    //         return true;
    ...
    // CASE 3: if we use both set and decrement

    for(ll i=n-1; i>= max(1LL, n-operations); i--){
        sum -= a[i];
        ll op_left = operations - (n-i);
        ll temp_sum = sum - a[0] + (n-i+1) * (a[0] - op_left);
        if(temp_sum <= k){
            return true;
        }
    }
