...
void solve() {
    ...
    // Three options:
    // dp[i] = d[i-1] + 20 --> if we buy a 20 Rs ticket on the current idx, and we add the optimal cost to reach i-1
    // dp[i] = 50 + dp[i-idx] --> where idx is the first index such that a[idx] < a[i] - 90 -> we buy a 50 Rs ticket at idx and add the optimal cost to reach idx
    // dp[i] = 120 + dp[i-idx2] --> where idx2 is the first index such that a[idx2] < a[i] - 1440 -> we buy a 120 Rs ticket at idx2 and add the optimal cost to reach idx2
    vll dp(n, INF); 

    for(ll i=0; i<n; i++){
        ll idx1 = upper_bound(a.begin(), a.begin()+i, a[i] - 90) - a.begin();
        ll idx2 = upper_bound(a.begin(), a.begin()+i, a[i] - 1440) - a.begin();

        dp[i] = (i ? dp[i-1] : 0) + 20;
        dp[i] = min(dp[i], (idx1 ? dp[idx1-1] : 0) + 50);
        dp[i] = min(dp[i], (idx2 ? dp[idx2-1] : 0) + 120);
