...
bool f(ll m, vector<ll>& a, ll c1, ll x, ll c2, ll y, ll k){
    // **DEBUG:** Compute lcm correctly instead of using x*y
    ll l = lcm(x, y);
    
    // **DEBUG:** Count positions with both contributions
    ll cnt_both = m / l;
    // **DEBUG:** Count positions with only the first discount (c1)
    ll cnt_c1 = m / x - cnt_both;
    // **DEBUG:** Count positions with only the second discount (c2)
    ll cnt_c2 = m / y - cnt_both;
    
    ll sum = 0;
    int idx = 0;
    // **DEBUG:** Apply both discount (c1+c2) to highest prices
    for (int i = 0; i < cnt_both; i++){
        sum += (a[idx++] / 100LL) * (c1 + c2);
    }
    // **DEBUG:** Apply discount c1 only
    for (int i = 0; i < cnt_c1; i++){
        sum += (a[idx++] / 100LL) * c1;
    }
    // **DEBUG:** Apply discount c2 only
    for (int i = 0; i < cnt_c2; i++){
        sum += (a[idx++] / 100LL) * c2;
    }
    
    // **DEBUG:** Check if achieved contribution is at least k
    return sum >= k;
}
...
void solve() {
    ...
    // **DEBUG:** Sort ticket prices in descending order
    ...
    // **DEBUG:** Ensure discount with the larger percentage is applied first
    if(c2 > c1){
        swap(c1, c2);
        swap(x, y);
    }
