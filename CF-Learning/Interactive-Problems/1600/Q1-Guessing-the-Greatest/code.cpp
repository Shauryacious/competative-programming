...
void solve() {
    ...
    while (lo < hi) {
        ll mid = (lo + hi) / 2;
        ll sec_max = ask(lo, hi); // Position of 2nd max in [lo, hi]
 
        if (sec_max <= mid) { // 2nd max in left
            if (ask(lo, mid) == sec_max) hi = mid; // max is also in left
            else lo = mid + 1;
        } else { // 2nd max in right
            if (ask(mid + 1, hi) == sec_max) lo = mid + 1; // max is also in right
            else hi = mid;
        }
    }
