...
ll ask(ll lo, ll mid) {
    ...
    ll sum;
    cin >> sum;  // Read the interactor's response
    return sum;
}

bool f(ll mid, ll lo, vll &prefix) {
    ll expected_sum = prefix[mid] - prefix[lo - 1];
    ll actual_sum = ask(lo, mid);
...
void solve() {
    ...
    while (lo <= hi) {
        ...
        if (f(mid, lo, prefix)) {
            ...
            hi = mid - 1;  // Move left
            ans = mid;      // Possible special stone location
        }
    }
