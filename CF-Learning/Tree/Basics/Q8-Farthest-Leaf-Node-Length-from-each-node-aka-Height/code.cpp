...
void dfs(ll u, ll p, vvll& adj, vll& farthestLeaf){
    for(auto v : adj[u]){
        if(v != p){
            dfs(v, u, adj, farthestLeaf);
            farthestLeaf[u] = max(farthestLeaf[u], farthestLeaf[v] + 1);
        }
    }
}
...
void solve() {
    ...
    vll farthestLeaf(n, 0);
    dfs(0, -1, adj, farthestLeaf);
    for(auto i : farthestLeaf){
        cout << i << " ";
    }
}
