...
void dfs1(ll u, ll p, vvll& adj, vll& isInPath){
    for(auto v : adj[u]){
        if(v != p){
            dfs1(v, u, adj, isInPath);
            isInPath[u] |= isInPath[v]; 
        }
    }
}

ll dfs2(ll u, ll p, vvll& adj) {
...
void solve() {
    ...
    debug(adj);
    ll total_paths = n * (n-1);

    vector<ll> isInPath(n, 0);
    isInPath[b] = 1; // b is in the path i.e. destination
    dfs1(f, -1, adj, isInPath); // f is the root of the tree
    debug(isInPath);

    ll x = 1;
    for(auto v : adj[f]){
        if(isInPath[v] != 1){ //not in path
            x += dfs2(v, f, adj);
        }
    }
    ...
    ll invalidPath = x*y;
    ll validPath = total_paths - invalidPath;
    cout<<validPath<<nl;
}
