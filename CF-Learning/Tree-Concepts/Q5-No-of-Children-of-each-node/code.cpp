...
// Solution:
// for root node => no of children = size of adj[root]
// for other nodes => no of children = size of adj[node] - 1
// since each non root node has a parent, so we subtract 1 from the size of adj[node]
...
void solve() {
    ...
    vll childrenCount(n, 0);
    childrenCount[0] = adj[0].size();
    for(ll i = 1; i < n; i++){
        childrenCount[i] = adj[i].size() - 1;
    }
