...
ll LOG = 20; // log2(10^6) = 20

void dfs(ll u, ll p, vvll& adj, vll& parent){
    parent[u] = p;
    for(auto v : adj[u]){
        if(v != p){
            dfs(v, u, adj, parent);
...
void precompute(ll n, vvll& adj, vll& parent){
    anc.resize(n, vector<ll>(LOG, 0));
...
// Why i+1? in anc[ans][i+1]
// Because anc[u][1] = parent[u]
// anc[u][2] = 2nd ancestor of u
// anc[u][3] = 4th ancestor of u
