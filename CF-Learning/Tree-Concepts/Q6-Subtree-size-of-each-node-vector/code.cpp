...
// Solution:
// Base Case:
// for leaf node, subtreeSize[node] = 1
// for non leaf node, subtreeSize[node] = 1 + sum of subtreeSize[child] for all child of node


// Time Complexity: O(N)
// Space Complexity: O(N)

void dfs(ll u, ll p, vvll& adj, vll& subtreeSize){
    subtreeSize[u] = 1;
    for(auto v : adj[u]){
        if(v != p){
            dfs(v, u, adj, subtreeSize);
