...
ll dfs(ll u, ll p, ll x, vvll& adj, ll& cuts){
    ll size = 1; //subtree size
    for(auto v : adj[u]){
        if(v != p){
            size += dfs(v, u, x, adj, cuts); 
        }
    }
...
bool f(ll x, ll k, vvll& adj){
    ll cuts = 0;
    dfs(0, -1, x, adj, cuts);
    return (cuts-1) >= k;
    // why cuts-1? because we need to cut k edges
    // but we get an extra cut because of the root node
}
...
void solve() {
    ...
    while(lo <= hi){
        ...
        if(f(mid, k, adj)){
            ans = mid;
            lo = mid+1; // if we can cut more than k edges then we can have greater value of x
        }
        else{
            hi = mid-1;
        }
