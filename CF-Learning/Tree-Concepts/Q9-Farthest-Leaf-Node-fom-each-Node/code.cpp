...
void dfs(ll u, ll p, vvll& adj, vpll& farthestLeafNode){
    // non root leaf node
    if(adj[u].size() == 1 && p != -1){
        farthestLeafNode[u] = {0, u};
        return;
    }
    for(auto v : adj[u]){
        if(v != p){
            dfs(v, u, adj, farthestLeafNode);
            if(farthestLeafNode[u].ff < farthestLeafNode[v].ff + 1){
                farthestLeafNode[u].ff = farthestLeafNode[v].ff + 1;
                farthestLeafNode[u].ss = farthestLeafNode[v].ss;
            }
        }
    }
}
...
void solve() {
    ...
    vector<pair<ll, ll>> farthestLeafNode(n, {-1, -1}); 
    // {farthestLeafNodeDistance, farthestLeafNode}
    dfs(0, -1, adj, farthestLeafNode);
    for(ll i=0; i<n; i++){
        cout<<i+1<<" => "<<farthestLeafNode[i].ff<<" "<<farthestLeafNode[i].ss+1<<nl;
    }
    cout<<nl;
}
