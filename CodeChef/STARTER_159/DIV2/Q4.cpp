...
// Function to determine if the player who starts first wins
int simulate_game(vll &piles) {
    ll n = piles.size();
    
    // XOR-sum for Nim game logic
    ll xor_sum = 0;
    for (ll stones : piles) {
        xor_sum ^= stones;
    }
    
    // If xor_sum is 0, the player who starts (Alice) will lose if both play optimally
    if (xor_sum == 0) {
        return 0;  // starting player loses
    } else {
        return 1;  // starting player wins
    }
}
...
void solve() {
    ...
    ll result = simulate_game(a);
    debug(result);
    ll num = d+1;
    ll den = 2*d;
    if(result == 1){
        cout<<mod_div(num, den, M)<<nline;
    }
    else{
        cout<<mod_div(num-2, den, M)<<nline;
    }
}
