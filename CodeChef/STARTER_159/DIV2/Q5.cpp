...
void solve() {
    ...
    // Cyclic loop on string
    ll turnR = 0;
    ll turnB = 0;
    ll x = min(n, m);
    while (x>0) {  // Assuming we want to cycle 'min(n, m)' times
        char ch = s[i];
        if (ch == 'A') {
            while (s[i] == 'A' && x>0) {  // Directly use s[i] to check condition
                countR += (n + m - 1);
                countR -= 2 * turnR;
                turnR++;
            ...
            i = (i - 1 + k) % k;  // Move back to the last character
        } else if (ch == 'B') {
            while (s[i] == 'B' && x>0) {  // Directly use s[i] to check condition
                countB += (n + m - 1);
                countB -= 2 * turnB;
                turnB++;
