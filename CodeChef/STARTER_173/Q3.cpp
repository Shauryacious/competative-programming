    ...
    ll booth_algorithm(const vector<ll>& b) {
        ll m = b.size();
        vector<ll> s = b;
        s.insert(s.end(), b.begin(), b.end()); // Concatenate B with itself
        vector<ll> f(m * 2, -1);
        ll k = 0;
        ...
        return k;  // Return the starting index of the smallest cyclic shift
    }
    ...
    public:
        Hash(vector<ll>& s) {
            ...
            for (ll i = 1; i <= n; i++) {
                ll curr_val = s[i - 1];
                hash[i] = {
                    (hash[i - 1].first * B1 + curr_val) % M,
                    (hash[i - 1].second * B2 + curr_val) % M
                };
                Bpower[i] = {
                    (Bpower[i - 1].first * B1) % M,
                    (Bpower[i - 1].second * B2) % M
                };
            }
        }
        ...
        pair<ll, ll> get(ll l, ll r) {
            ...
            ll len = r - l + 1;
            ll hash1 = (hash[r].first - (hash[l - 1].first * Bpower[len].first) % M + M) % M;
            ll hash2 = (hash[r].second - (hash[l - 1].second * Bpower[len].second) % M + M) % M;
            return {hash1, hash2};
        }
    };
    ...
    void solve() {
        ...
        ll start = booth_algorithm(b);
        vector<ll> rotated_b(b.begin() + start, b.end());
        rotated_b.insert(rotated_b.end(), b.begin(), b.begin() + start);
        b = rotated_b;
        debug(b);
        ...
        pair<ll, ll> h1 = hb.get(0, m-1);

        for (ll i = 0; i <= n - m; i++) {
            if (ha.get(i, i + m - 1) == h1) continue; // Skip identical segments
            
            ll l = 0, r = m - 1, first_diff = -1;
            
            while (l <= r) {
                ll mid = (l + r) / 2;
                if (ha.get(i, i + mid) == hb.get(0, mid)) {
                    l = mid + 1;
                } else {
                    first_diff = mid;
                    r = mid - 1;
            ...
            if (first_diff != -1 && a[i + first_diff] > b[first_diff]) {
                for (ll j = 0; j < m; j++) {
                    a[i + j] = b[j];
                }
            }
