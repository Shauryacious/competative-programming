...
void solve() {
    ...
    ll result = 1; // Start with the neutral element for multiplication.
    bool possible = true;
    ...
    for (ll i = 0; i < n && possible; ++i) {
        for (ll j = i + 1; j < n; ++j) {
            ll xor_val = a[i] ^ a[j];
            if (xor_val > r) {
                possible = false; // If XOR exceeds r, terminate.
                break;
            }
            result *= xor_val;
            if (result > r) {
                possible = false; // Prevent overflow and excess computation.
                break;
            }
        }
    }

    if (possible && result >= l && result <= r) {
        cout << "YES\n";
    } else {
        cout << "NO\n";
    }
