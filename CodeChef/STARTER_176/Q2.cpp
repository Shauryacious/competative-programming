...
void solve() {
    ...
    ll cntc = count(s.begin(), s.end(), 'C');
    ll cntd = count(s.begin(), s.end(), 'D');
    ll cnte = count(s.begin(), s.end(), 'E');

    vector<vector<ll>> dp(32, vector<ll>(n+1, 0));

    // 
    for(ll i=0; i<32; i++){
        dp[i][0] = __builtin_popcount(i)*c;
    }
